					...
					<?php
						if (isset($title) && isset($picture) && isset($link) && isset($link_log)){
							...
							if(!move_uploaded_file($_FILES['picture']['tmp_name'], '../' . $path)){
								...
								$kek = "$path$picture";
							$result = mysqli_query($db, "UPDATE languages SET title='$title', picture='$picture', link='$link', link_log='$link_log' WHERE id='$id'"); 
							if($result == 'true'){
								echo "<p>Ваш язык успешно обновлен!</p>";
							}else{
								echo "<p>Ваш язык не обновлен!</p>";
							}
						}else{
