...
$result = mysqli_query($db, "SELECT title FROM `languages` WHERE id='5'");
...
<body>
	...
	<main>
		...
		<?php 
			$result = mysqli_query($db, "SELECT id, title, cost, description_short, description_long, link, link_log, image FROM `courses` WHERE language='Итальянский язык'");
			...
			do{
				printf('
					...
					', $myrow["image"], $myrow["title"], $myrow["description_short"], $myrow["link"], $myrow["description_long"]);
			}
